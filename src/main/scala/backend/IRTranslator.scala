...
//  var scratchRegs: List[Register] = List(new scratchReg("foo1"), new scratchReg("foo2"), new scratchReg("foo3"), new scratchReg("foo4"),new scratchReg("foo5"))
  ...
  private def translateStatements(stmts:List[Stat], symbolTable: mutable.Map[String, frontend.ast.Type]):Unit = {
    ...
    for (stmt <- stmts if !reachedRestBlock) {
      ...
      instructions = instructions.concat(stmt match {
        ...
        case Return(expr) => evaluateExpr(expr, ReturnRegister(), BIT_64)
        case Exit(expr) => {
          addBlock(ExitBlock())
      ...
      case IntType() | BoolType() | CharType() | StringType() => RValue match {
        ...
        case Call(name, args) => {
          ...
          for (arg <- args) {
            ...
            paramRegs += paramReg
            paramCount += 1
            moveParams = moveParams.concat((evaluateExpr(arg, ReturnRegister(), BIT_64).concat(ListBuffer(Push(paramReg), MovInstr(ReturnRegister(), paramReg)))))
          }
  ...
  def evaluateExpr(expr: Expr, reg:Register, size: Size): ListBuffer[Instruction] = {
    expr match {
      ...
      case Ident(name) => ListBuffer(MovInstr(varMap(name), reg).changeSize(size))
      case Neg(x) => evaluateExpr(new Sub(IntLit(0)(nullPos), x)(nullPos), reg, size)
      case Chr(x) => {
        addBlock(errBadChar())
        var instrs = ListBuffer() : ListBuffer[Instruction]
        x match {
          case IntLit(x) =>
            instrs.concat(List(MovInstr(Immediate(x), reg).changeSize(size)))
          case _ =>
            instrs.concat(evaluateExpr(x, reg, BIT_64))
        }
        instrs.concat(List(
          TestInstr(Immediate(-128), reg).changeSize(size),
      ...
      case Mod(x, y) => {
        val yReg = new scratchReg(scratchCounter, 0)
        scratchCounter += 1
        val instrs = evaluateExpr(x, reg, size).concat(evaluateExpr(y, yReg, size)).addOne(ModInstr(reg, yReg).changeSize(size))
        scratchCounter = 0
        instrs
      }
      case GT(x, y) => {
