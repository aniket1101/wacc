...
class ArrayTest extends AnyFlatSpec{
  ...
  it should "parse nested arrays" in {
    parser.parse(format("bool[] a = [0, 0, 1]; bool[][] b = [a]")) shouldBe Right(Prog(List(), Stmts(Declaration(ArrayType(BoolType()),Ident("a"),ArrayLit(List(IntLit(0), IntLit(0), IntLit(1)))), Declaration(ArrayType(ArrayType(BoolType())),Ident("b"),ArrayLit(List(Ident("a")))))))
