...
//         |end
//       """.stripMargin
//     parser.parse(input) shouldBe Right(Prog(List(),Stats(Declaration(IntType(),Ident("result"),Call(Ident("foo"),ArgList(List()))),Stats(Declaration(StringType(),Ident("name"),Call(Ident("bar"),ArgList(List(CharLit('A'))))),Declaration(BoolType(),Ident("cond"),Call(Ident("foobar"),ArgList(List(BoolLit(true), StrLit("True")))))))))
//   }
